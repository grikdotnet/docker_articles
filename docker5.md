...
В контейнерах обычно есть свои пользователи и группы со своими uid/gid, а значения uid/gid монтируемых каталогов сохраняются те, которые были присвоены в host-системе.
У группы docker, которая создатся в host-системе, gid равен 999. В образе mysql-server группа с gid 999 называется ssl, а все файлы открываются под пользователем mysql с uid 27.
С большой вероятностью возникнут проблемы с правами доступа к подключаемому каталогу.

Надеюсь, когда-нибудь для официальных образов docker примут соглашение, по которому во всех образах будет стандартная группа docker с тем же gid, что и в host-системе.
Пока что придется создать специальную группу для томов и добавить себя в нее.
