...
Создавать виртуальную машину и настраивать лучше из командной строки, не через графическую обертку.
...
Если вкратце, у Docker несколько драйверов для работы с файловой системой, обычно это [AUFS](https://en.wikipedia.org/wiki/Aufs), и все файлы контейнеров лежат в /var/lib/docker/aufs/diff/.
...
```
Пишут, что у образ nginx 1.9.4 размером 52 мб, а по факту, у меня качает он всего 3 мб. Это потому, что Nginx собран на образе `debian:jessie`, который у меня "Already exists".
...
Контейнеры создаются командами `docker run` и `docker create`. 
