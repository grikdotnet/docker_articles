...
Разобраться к чему относятся тома в /var/lib/docker/volumes/ достаточно сложно, и при удалении контейнеров они остаются. Это водит к замусориванию и потере места на диске. В документации пишут что команда [работает над проблемой](https://github.com/docker/docker/issues/14214).
Удобнее прямо указывать какие каталоги надо монтировать, чтобы можно было просматривать их содержимое и удалять когда они станут не нужны.
