...
A common moot point is whether Nginx configuration files are a part of a web application. System administrators are disputing with application developers on permissions.
But a few years ago came the devops guys, and wished to replace a conventional procedural command calls from bash with an OOP design applied to the infrastructure.
